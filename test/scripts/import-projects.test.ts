...
describe('Import projects script', () => {
  ...
  it('succeeds to import Gitlab project from file', async () => {
    ...
    // Gitlab project
    expect(logFile).toMatch(
      `"target":{"name":"vulnmap-api-import-tool-test-org/project-from-template","branch":"main"}`,
    );
    discoveredProjects.push(...projects);
  }, 2400000);
});
...
describe('Error handling', () => {
  ...
  it('shows correct error when VULNMAP_LOG_PATH is not set', async () => {
    delete process.env.VULNMAP_LOG_PATH;
    expect(
      importProjects(
        path.resolve(
          __dirname +
      ...
      'Please set the VULNMAP_LOG_PATH e.g. export VULNMAP_LOG_PATH',
    );
  }, 300);
});
...
describe('No projects scenarios', () => {
  ...
  it('succeeds to complete import targets from empty repo', async () => {
    ...
    const logFiles = generateLogsPaths(logPath, ORG_ID);
    logs = Object.values(logFiles);
    process.env.VULNMAP_LOG_PATH = logPath;
