...
describe('Logs failed polls', () => {
  ...
  it('Fails to import targets from file', async () => {
    ...
    expect(failedLog).toMatch(
      `"level":50,"orgId":"ORG-ID","locationUrl":"https://app.vulnmap.khulnasoft.com/api/v1/org/ORG-ID/integrations/INTEGRATION-ID/import/IMPORT-ID","errorMessage":{"statusCode":500,"error":{"message":"Error calling Vulnmap api"}},"msg":"Failed to poll url"`,
    );
  }, 240000);
});
