...
describe('buildGitCloneUrl', () => {
  ...
  it('builds correct clone url for github.com / ghe (the urls come back from API already correct)', async () => {
    ...
    const url = github.buildGitCloneUrl({
      branch: 'main',
      archived: false,
      sshUrl: 'https://git@github.com:khulnasoft-lab/vulnmap-api-import.git',
      cloneUrl: 'https://github.com/khulnasoft-lab/vulnmap-api-import.git',
    });
    expect(url).toEqual(
      `https://secret_token:x-oauth-basic@github.com/khulnasoft-lab/vulnmap-api-import.git`,
    );
  });
});
