...
const requestWithRateLimitHandlingStub = jest
  ...
  .mockResolvedValue({
    statusCode: 201,
    headers: { location: 'https://app.vulnmap.khulnasoft.com/location' },
  });
...
describe('importTarget()', () => {
  ...
  process.env.VULNMAP_API = process.env.VULNMAP_API_TEST as string;
  process.env.VULNMAP_TOKEN = process.env.VULNMAP_TOKEN_TEST;
  process.env.VULNMAP_LOG_PATH = path.resolve(__dirname + `/fixtures/`);
