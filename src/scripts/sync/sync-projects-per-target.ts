...
const debug = debugLib('vulnmap:sync-projects-per-target');
...
export function getTargetConverter(
  origin: SupportedIntegrationTypesUpdateProject,
): (target: VulnmapTarget) => Target {
  const getTargetConverter = {
    [SupportedIntegrationTypesUpdateProject.GITHUB]: vulnmapTargetConverter,
    [SupportedIntegrationTypesUpdateProject.GHE]: vulnmapTargetConverter,
  };
  ...
  if (!dryRun) {
    for (
      ...
      failedProjects.map((f) => {
        failed.push({
          ...
          dryRun,
          errorMessage:
            f.userMessage ?? 'Failed to import project via Vulnmap Import API',
        });
      });
    }
    return { updated, failed };
    ...
    files.map((f) =>
      updated.push({
        ...
        type: ProjectUpdateType.IMPORT,
        from: f,
        to: `https://app.vulnmap.khulnasoft.com/org/example-org-name/project/example-project-id-uuid`,
        dryRun,
      }),
    );
...
export function vulnmapTargetConverter(target: VulnmapTarget): Target {
