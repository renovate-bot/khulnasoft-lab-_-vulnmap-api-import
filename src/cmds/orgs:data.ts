...
const debug = debugLib('vulnmap:orgs-data-script');
...
export const builder = {
  sourceOrgPublicId: {
    required: false,
    default: undefined,
    desc: 'Public id of the organization in Vulnmap that can be used as a template to copy all supported organization settings.',
  },
  groupId: {
    required: true,
    default: undefined,
...
export async function generateOrgImportData(
  ...
  try {
    ...
    const errorMessage = `ERROR! Failed to generate data. Try running with \`DEBUG=vulnmap* <command> for more info\`.\nERROR: ${e.message}`;
    return {
      fileName: undefined,
      exitCode: 1,
